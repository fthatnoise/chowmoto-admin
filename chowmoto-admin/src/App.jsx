    ...
    async function load() {
      try {
        ...
        if (snap.exists() && snap.data().list?.length > 0) {
          // Merge: seed has all fields (plusOne, plusOneName); Firebase has saved responses
          const saved = snap.data().list;
          const savedById = Object.fromEntries(saved.map(g => [g.id, g]));
          const merged = SEED_GUESTS.map(seed => {
            const fb = savedById[seed.id];
            if (!fb) return seed;
            return {
              ...seed,
              venues: fb.venues ?? seed.venues,
              plusOneConfirmed: fb.plusOneConfirmed,
              contactEmail: fb.contactEmail ?? seed.email ?? "",
